...
object MethodSignatureWrapper {
  ...
  def apply(returnType: Type, name: String, paramTypes: RefArray[FieldType], isStatic: Boolean): MethodSignatureWrapper = {
  ...
  def apply(ii: MethodInvocationInstruction): MethodSignatureWrapper = {
    new MethodSignatureWrapper(ii.methodDescriptor.returnType, ii.name, ii.methodDescriptor.parameterTypes, ii.isInstanceOf[INVOKESTATIC])
  }
  ...
  def apply(sig: String): MethodSignatureWrapper = {
    ...
    if (params.isEmpty || (params.size == 1 && params(0).isEmpty)) {
      new MethodSignatureWrapper(applyReturn(sig1.substring(0, sig1.indexOf(' '))),
        sig1.substring(sig1.indexOf(' ') + 1, sig1.indexOf('(')).trim, RefArray.empty[FieldType], sig.contains("static "))
    }
    else
      new MethodSignatureWrapper(applyReturn(sig1.substring(0, sig1.indexOf(' '))),
        sig1.substring(sig1.indexOf(' ') + 1, sig1.indexOf('(')).trim,
        RefArray.from(params.map(p => applyParam(p)).toArray), sig.contains("static "))
  }
...
class MethodSignatureWrapper(val returnType: Type, val name: String, val paramTypes: RefArray[FieldType], val isStatic: Boolean) {


  def this(m: org.opalj.br.Method) {
    this(m.returnType, m.name, m.parameterTypes, m.isStatic)
  }

  override def toString: String = (if (isStatic) "static " else "") + returnType.toJava + " " + name + "(" + paramTypes.toList.map(_.toJava).mkString(",") + ")"
